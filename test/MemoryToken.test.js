...
contract('Memory Token', (accounts) => {
  ...
  describe('token distribution', async () => {
    ...
    it('mints tokens', async () => {
      await token.mint(accounts[0], 'https://www.google.com')
      ...
      let expected = ['1']
      assert.equal(tokenIds.toString(), expected.toString(), 'tokemIds are correct')
      
      // Token URI correct
      let tokenURI = await token.tokenURI('1')
      assert.equal(tokenURI,'https://www.google.com')
